...
@Component({
  selector: 'app-trips-ongoing',
  standalone: true,
  imports : [
    HttpClientModule, TableModule, CommonModule, RouterLink , ButtonComponent,ButtonModule, Button, ModalComponent, LoaderComponent
  ],
  templateUrl: './trips-ongoing.component.html',
  styleUrl: './trips-ongoing.component.scss'
})
export class TripsOngoingComponent implements OnInit {
  ...
  moreActions:boolean = false;
   ...
   ngOnInit(): void {
     this.getAllTrips().subscribe(
       (res:any) =>{
         console.log('ongoing trips',res.data.trips.data)
   ...
   getAllTrips(): Observable<any>{
     return this.http.get<any>(`${this.baseUrl}/api/admin/trips/ongoing`);
   }
